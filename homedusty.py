...
thingurl_pm25="https://api.thingspeak.com/update?api_key=JYQPWHB0ENBYQ3TP&field1="
PM10="&field3="
#thingurl_pm10="https://api.thingspeak.com/update?api_key=JYQPWHB0ENBYQ3TP&field3="

ddev = "/dev/ttyUSB0"   #  device name 
debug = 2      # debug level in sds011 class module
cycles = 5      # serial read timeout in seconds, dflt 2
timeout = 10     # timeout on serial line read
time_sleep = 300
DC = 2
...
if len(sys.argv) < 2:
    ...
    sys.exit("Usage: python3 test.py [-d 1] [-c 10] [-t 10] [-u 1] com_port [duty_cycle 0]\n"
             "com port e.g. /dev/ttyUSB0 -u 1\n"
...
def printThing(values):
   try:
      url = thingurl_pm25 + str(values[1]) + PM10 + str(values[0])
      print(url) 
...
#    while True: 
        ...
        print("Read was succesfull. Going to sleep for %d  seconds", time_sleep)
